...
async def main ():
    ...
    if args.verbosity: print(f"ⓘ args: {args}", file=sys.stderr)
    ...
    if args.verbosity: print(f"ⓘ Found {len(urls_data)} unique URLs from {len(tags)} tags", file=sys.stderr)
    if args.verbosity: print(f"ⓘ Unique tag attributes: {sorted(await get_unique_tag_attrs(tags))}", file=sys.stderr)
