...
SELECT
  ...
  ROUND( TO_CHAR( ( ( SUM(failed_count) / SUM(passed_count + failed_count) ) * 100 ) ), 0 ) AS failed_pct,
  ROUND( TO_CHAR( SUM(total_response_time) / SUM(passed_count+failed_count) ), 0 ) AS total_response_time,
