...
Inverse function of localtime(). Its argument is the struct_time or full 9-tuple (since the dst flag is needed; use -1 as the dst flag if it is unknown) which expresses the time in local time, not UTC. It returns a floating-point number, for compatibility with time(). 
...
# Times
FS_HHMM         | { FS_HHMM         } | { time.strftime(FS_HHMM        ) }
FS_HH_MM        | { FS_HH_MM        } | { time.strftime(FS_HH_MM       ) }
FS_HHMMSS       | { FS_HHMMSS       } | { time.strftime(FS_HHMMSS      ) }
FS_HH_MM_SS     | { FS_HH_MM_SS     } | { time.strftime(FS_HH_MM_SS    ) }
FS_HHMMSS_SSS   | { FS_HHMMSS_SSS   } | { time.strftime(FS_HHMMSS_SSS  ) }
FS_HH_MM_SS_SSS | { FS_HH_MM_SS_SSS } | { time.strftime(FS_HH_MM_SS_SSS) }

# Time Zones: {FS_TZ_UTC}Z | {FS_TZ_OFFSET}±hh:mm {FS_TZ_NAME}±hh

# ISO8601
FS_ISO8601_DATE | { FS_ISO8601_DATE } | { time.strftime(FS_ISO8601_DATE) }
FS_ISO8601_TIME | { FS_ISO8601_TIME } | { time.strftime(FS_ISO8601_TIME) }
FS_ISO8601_UTC  | { FS_ISO8601_UTC  } | { time.strftime(FS_ISO8601_UTC ) }
FS_ISO8601_DT   | { FS_ISO8601_DT   } | { time.strftime(FS_ISO8601_DT  ) }

# Other Date & Time formats
FS_ATOM         | { FS_ATOM         } | { time.strftime(FS_ATOM        ) }
FS_COOKIE       | { FS_COOKIE       } | { time.strftime(FS_COOKIE      ) }
FS_RSS          | { FS_RSS          } | { time.strftime(FS_RSS         ) }
FS_W3C          | { FS_W3C          } | { time.strftime(FS_W3C         ) }
FS_RFC822       | { FS_RFC822       } | { time.strftime(FS_RFC822      ) }
FS_RFC850       | { FS_RFC850       } | { time.strftime(FS_RFC850      ) }
FS_RFC1036      | { FS_RFC1036      } | { time.strftime(FS_RFC1036     ) }
FS_RFC1123      | { FS_RFC1123      } | { time.strftime(FS_RFC1123     ) }
FS_RFC2822      | { FS_RFC2822      } | { time.strftime(FS_RFC2822     ) }
FS_RFC3339      | { FS_RFC3339      } | { time.strftime(FS_RFC3339     ) }
"""
)
...
print(
    f"""
time.strftime('%Y-%m-%d')      | { time.strftime('%Y-%m-%d')      } | Display now as year-month-day
time.strftime('%H:%M:%S')      | { time.strftime('%H:%M:%S')      } | Display now as hour:min:sec
time.strftime(FS_ISO8601_DATE) | { time.strftime(FS_ISO8601_DATE) } | Display now as date only
time.strftime(FS_ISO8601_TIME) | { time.strftime(FS_ISO8601_TIME) } | Display now as time only
time.strftime(FS_ISO8601_UTC)  | { time.strftime(FS_ISO8601_UTC)  } | Display now as UTC
"""
)
