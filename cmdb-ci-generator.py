        ...
        items.append(
            {
                ...
                "MAC": faker.mac_address().upper(),
                "Endpoint-IPv4-Static": faker.ipv4_private() if faker.boolean(0.1) else "",
...
def load_IEEE_OUIs():
    ...
    if not os.path.exists(IEEE_OUI_CSV_FILENAME):
        if args.verbosity:
            print(f"ⓘ   Parsing base16 IEEE OUIs ...", file=sys.stderr)
    ...
    if os.path.exists(IEEE_OUI_CSV_FILENAME):
        if args.verbosity:
            print(f"ⓘ   Reading cached CSV of IEEE OUIs", file=sys.stderr)
...
def load_dataframes():
    ...
    """
    if args.verbosity:
        print(f"ⓘ load_dataframes()", file=sys.stderr)
...
def show(resources=None, resource_name=None, format="pretty", filename="-"):
    ...
    if args.verbosity:
        print(f"ⓘ show(): {len(resources)} resources of type {type(resources[0])} as {format}", file=sys.stderr)
...
def main():
    ...
    if args.verbosity >= 3:
        print(f"ⓘ Args: {args}")
    if args.verbosity:
        print(f"ⓘ filename: {'- (stdout)' if args.filename == '-' else args.filename}", file=sys.stderr)
    if args.verbosity:
        print(f"ⓘ format: {args.format}", file=sys.stderr)
    if args.verbosity:
        print(f"ⓘ number: {args.number}", file=sys.stderr)
    if args.verbosity:
        print(f"ⓘ timer: {args.timer}", file=sys.stderr)
    if args.verbosity:
    ...
    if args.verbosity:
        print(f"ⓘ Loading DataFrames ...", file=sys.stderr)
    load_dataframes()

    if args.verbosity:
        print(f"ⓘ Generating: {args.number} items ...", file=sys.stderr)
    items = generate_cmdb_ci(args.number)
    # items = generate_cmdb_ci_6_columns(args.number)

    if args.verbosity:
        print(f"ⓘ Total Config Items = {len(items)}", file=sys.stderr)
